...
{
  echo "# generated by ${SOURCE%.git}/blob/$BRANCH_NAME/docker-publish.sh"
  echo "FROM python:3.14-slim"
  echo "LABEL maintainer=\"$MAINTAINER\""
  echo "LABEL description=\"Telegram bot for study purposes\""
  echo "LABEL version=\"$VERSION\""
  echo "LABEL org.opencontainers.image.source=\"$SOURCE\""
  grep -v '^\s*#' Dockerfile.base
} | docker build -t "$DOCKER_USER/$IMAGE_NAME:$TAG" -f- .
